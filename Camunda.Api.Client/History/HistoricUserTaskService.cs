...
{
  private readonly IHistoricUserTaskRestService _api;

  internal HistoricUserTaskService(IHistoricUserTaskRestService api) => _api = api;
  ...
  /// <returns></returns>
  public Task<List<DurationReportResult>> GetDurationReport(HistoricTaskDurationReport historicTaskReport) 
    => _api.GetDurationReport(historicTaskReport);
  ...
  /// <param name="historicTaskReport"></param>
  /// <returns></returns>
  public Task<List<CountReportResult>> GetCountReport(HistoricTaskCountReport historicTaskReport) 
    => _api.GetCountReport(historicTaskReport);
}