...
public class HistoricVariableInstanceResource
{
  ...
  public Task<HistoricVariableInstance> Get(bool deserializeValue = true) 
    => _api.Get(_variableId, deserializeValue);

  /// <summary>
  /// Retrieves the content of a historic variable by id. Applicable for variables that are serialized as binary data.
  /// </summary>
  /// <returns></returns>
  public async Task<HttpContent> GetBinary() 
    => (await _api.GetBinaryVariable(_variableId)).Content;
}