...
public class ProcessDefinitionService
{
  ...
  public ProcessDefinitionResource this[string processDefinitionId]
    => new ProcessDefinitionResourceById(_api, processDefinitionId);

  public ProcessDefinitionResource ByKey(string processDefinitionKey)
    => new ProcessDefinitionResourceByKey(_api, processDefinitionKey);

  public ProcessDefinitionResource ByKey(string processDefinitionKey, string tenantId)
    => new ProcessDefinitionResourceByKeyAndTenantId(_api, processDefinitionKey, tenantId);
  ...
  /// <param name="includeFailedJobs">Whether to include the number of failed jobs in the result or not.</param>
  /// <remarks>This does not include historic data.</remarks>
  public Task<List<ProcessDefinitionStatisticsResult>> GetStatistics(bool includeFailedJobs)
    => GetStatistics(includeFailedJobs, false);
  ...
  /// <returns></returns>
  public Task Delete(string processDefinitionId, bool cascade, bool skipCustomListeners, bool skipIoMappings = true)
    => _api.Delete(processDefinitionId, cascade, skipCustomListeners, skipIoMappings);
}