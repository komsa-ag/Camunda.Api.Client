...
{
  private readonly IMigrationRestService _api;

  internal MigrationService(IMigrationRestService api) => _api = api;
  ...
  /// <returns></returns>
  public Task<MigrationPlan> Generate(MigrationPlanGeneration request)
  ...
  /// <returns></returns>
  public Task<MigrationPlanReport> Validate(MigrationPlan plan)
    => _api.Validate(plan);
  ...
  /// <param name="request"></param>
  /// <returns></returns>
  public Task Execute(MigrationExecution request)
  ...
  /// <param name="request"></param>
  /// <returns></returns>
  public Task<BatchInfo> ExecuteAsync(MigrationExecution request)
    => _api.ExecuteAsync(request);
}