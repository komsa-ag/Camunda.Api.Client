...
public class ApiException : Exception
{
    ...
    private static Func<string, string, HttpResponseMessage, ApiException> GetConstructor(string typeName)
    {
        return _knownTypes.GetOrAdd(typeName, typeName_ =>
        {
            ...
            ParameterExpression paramType = Expression.Parameter(typeof(string));
            ParameterExpression paramMessage = Expression.Parameter(typeof(string));
            ParameterExpression paramResponse = Expression.Parameter(typeof(HttpResponseMessage));

            ConstructorInfo ctor = t.GetConstructor(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance, null, new[] { paramType.Type, paramMessage.Type, paramResponse.Type }, null);

            if (ctor == null)
                return null;

            NewExpression newExpression = Expression.New(ctor, paramType, paramMessage, paramResponse);
            Expression<Func<string, string, HttpResponseMessage, ApiException>> lambda = Expression.Lambda<Func<string, string, HttpResponseMessage, ApiException>>(newExpression, paramType, paramMessage, paramResponse);
    ...
    {
        Func<string, string, HttpResponseMessage, ApiException> ctor = GetConstructor(restError.Type);
        if (ctor != null)
