...
public class ExternalTaskResource
{
  ...
  public Task SetRetries(int retries) 
    => _api.SetRetries(_externalTaskId, new RetriesInfo() { Retries = retries });
  ...
  public Task SetPriority(long priority) 
    => _api.SetPriority(_externalTaskId, new PriorityInfo { Priority = priority });

  /// <summary>
  /// Complete an external task and update process variables.
  /// </summary>
  public Task Complete(CompleteExternalTask completeExternalTask) 
    => _api.Complete(_externalTaskId, completeExternalTask);
  ...
  /// </summary>
  public Task HandleFailure(ExternalTaskFailure externalTaskFailure) 
    => _api.HandleFailure(_externalTaskId, externalTaskFailure);
  ...
  /// </summary>
  public Task HandleBpmnError(ExternalTaskBpmnError externalTaskBpmnError)
    => _api.HandleBpmnError(_externalTaskId, externalTaskBpmnError);
  ...
  /// </summary>
  public Task ExtendLock(ExternalTaskExtendLock externalTaskExtendLock) 
    => _api.ExtendLock(_externalTaskId, externalTaskExtendLock);

  /// <summary>
  /// Locks the task by ID
  /// </summary>
  public Task Lock(ExternalTaskLock externalTaskLock) 
    => _api.Lock(_externalTaskId, externalTaskLock);
