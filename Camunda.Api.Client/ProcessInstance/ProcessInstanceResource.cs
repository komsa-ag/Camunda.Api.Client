...
public class ProcessInstanceResource
{
  ...
  private readonly IProcessInstanceRestService _api;
  ...
  public Task Delete(bool skipCustomListeners = false, bool skipIoMappings = false, bool skipSubprocesses = false)
    => _api.DeleteProcessInstance(_processInstanceId, skipCustomListeners, skipIoMappings, skipSubprocesses);

  /// <summary>
  /// Retrieves an Activity Instance (Tree) for a given process instance.
  /// </summary>
  public Task<ActivityInstanceInfo> GetActivityInstance()
    => _api.GetActivityInstanceTree(_processInstanceId);

  public Task UpdateSuspensionState(bool suspended)
    => _api.UpdateSuspensionStateForId(_processInstanceId, new() { Suspended = suspended });
  ...
  /// </remarks>
  public Task ModifyProcessInstance(ProcessInstanceModification modification)
    => _api.ModifyProcessInstance(_processInstanceId, modification);

  public VariableResource Variables => new(_api, _processInstanceId);
