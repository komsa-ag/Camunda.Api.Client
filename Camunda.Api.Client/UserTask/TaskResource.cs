...
public class TaskResource
{
  ...
  public TaskIdentityLinkResource IdentityLink => new(_api, _taskId);
  public TaskCommentResource Comment => new(_api, _taskId);
  public TaskAttachmentResource Attachment => new(_api, _taskId);
  public VariableResource Variables => new(_api, _taskId);
  public LocalVariableResource LocalVariables => new(_api, _taskId);
  ...
  ///  * If the task has Form Field Metadata defined, the process engine will perform backend validation for any form fields which have validators defined.
  /// </remarks>
  public Task<Dictionary<string, VariableValue>> SubmitFormAndFetchVariables(CompleteTask completeTask)
    => _api.SubmitFormTaskAndFetchVariables(_taskId, completeTask);
  ...
  /// </remarks>
  public Task Claim(string userId) => _api.ClaimTask(_taskId, new() { UserId = userId });
  ...
  /// </summary>
  public Task<Dictionary<string, VariableValue>> CompleteAndFetchVariables(CompleteTask completeTask)
    => _api.CompleteTaskAndFetchVariables(_taskId, completeTask);
  ...
  /// </summary>
  public Task Delegate(string delegatedUser) => _api.DelegateTask(_taskId, new() { UserId = delegatedUser });
  ...
  /// </remarks>
  public Task SetAssignee(string userId) => _api.SetAssignee(_taskId, new() { UserId = userId });
  ...
  /// </param>
  public Task<Dictionary<string, VariableValue>> GetFormVariables(params string[] variableNames)
    => _api.GetFormVariables(_taskId, variableNames.Join());
  /// <summary>
  /// Retrieves the form variables for a task. 
  /// The form variables take form data specified on the task into account. 
  /// If form fields are defined, the variable types and default values of the form fields are taken into account.
  ...
  public Task<Dictionary<string, VariableValue>> GetFormVariables(string[] variableNames, bool deserializeValues = true)
    => _api.GetFormVariables(_taskId, variableNames.Join(), deserializeValues);
