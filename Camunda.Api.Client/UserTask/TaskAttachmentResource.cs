...
public class TaskAttachmentResource
{
  ...
  /// </summary>
  public Task<List<AttachmentInfo>> GetAll()
    => _api.GetAttachments(_taskId);

  /// <summary>
  /// Retrieves a single task attachment by task id and attachment id.
  /// </summary>
  public Task<AttachmentInfo> Get(string attachmentId)
    => _api.GetAttachment(_taskId, attachmentId);

  /// <summary>
  /// Retrieves the binary content of a single task attachment by task id and attachment id.
  /// </summary>
  public async Task<HttpContent> GetData(string attachmentId)
    => (await _api.GetAttachmentData(_taskId, attachmentId)).Content;

  /// <summary>
  /// Removes an attachment from a task.
  /// </summary>
  public Task Delete(string attachmentId)
    => _api.DeleteAttachment(_taskId, attachmentId);
  ...
  public Task<AttachmentInfo> Create(string name, string description, string type, string url, AttachmentContent content)
    => _api.AddAttachment(_taskId, new("attachment-name", name), new("attachment-description", description),
          new("attachment-type", type), new("url", url), content);
