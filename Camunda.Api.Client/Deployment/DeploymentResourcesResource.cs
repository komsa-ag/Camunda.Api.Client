...
public class DeploymentResourcesResource
{
  ...
  /// </summary>
  public Task<List<DeploymentResourceInfo>> GetAll()
    => _api.GetDeploymentResources(_deploymentId);

  /// <summary>
  /// Retrieves a single deployment resource by resource id for the given deployment.
  /// </summary>
  public Task<DeploymentResourceInfo> Get(string resourceId)
    => _api.GetDeploymentResource(_deploymentId, resourceId);

  /// <summary>
  /// Retrieves the binary content of a single deployment resource for the given deployment.
  /// </summary>
  public async Task<HttpContent> GetData(string resourceId)
    => (await _api.GetDeploymentResourceData(_deploymentId, resourceId)).Content;
